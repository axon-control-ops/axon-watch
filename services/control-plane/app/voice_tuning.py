"""Shared voice-rate and pitch normalization helpers (axon-local parity)."""
...
def normalized_voice_rate(value: Any) -> float:
    return _normalized_voice_scalar(value, DEFAULT_VOICE_RATE, minimum=0.50, maximum=1.30)
...
def azure_voice_rate_attr(value: Any) -> str:
    """Azure SSML relative rate, e.g. 0.85 → '-15%' (axon-local parity)."""
...
def azure_voice_pitch_attr(value: Any) -> str:
    """Azure SSML relative pitch, e.g. 1.04 → '+4%'."""
