...
def _strip_stream_blocks(text: str) -> str:
    ...
    text = re.sub(r":::[a-z_]+\b.*?(?::::|\Z)", "", text, flags=re.DOTALL | re.IGNORECASE)
    return text.strip()
...
def _normalize_spoken_line(raw: str) -> str:
    text = _strip_stream_blocks(str(raw or ""))
    text = _dedupe_doubled_text(text)
    text = re.sub(r"^[\"'`]+|[\"'`]+$", "", text)
    text = re.sub(r"^(KAIRO|Jarvis)\s*[:—-]\s*", "", text, flags=re.IGNORECASE)
