...
### Lead continuous shift retry (approved-wrapper sandbox) — 2026-08-07
- owner: lead (`run_lead_retry_wrappers_2026_08_07`; this IDE retry; prior continuous failure: `Approved wrappers cannot come from the workspace`)
- commit: evidence append only (no commit/push requested)
- commands run: Read of `agent_sandbox.py` + Lead/role `execution_policy.py` + `install-bin-wrappers.sh` + prior Lead receipts; authoritative roster/team-health thread; `axon-agent-terminal-job --help` (wrapper present on PATH); `axon-agent-terminal-job --workspace workspace_axon_watch -- true` → enqueue rejected (`agent terminal run has no scoped task`); `git status`/`git log` rejected (workspace mount has no `.git`); raw `/health` + console probes denied by agent shell policy this turn
- pass/fail: pass for bounded Lead retry scope — wrapper failure root-caused from local code; fleet coach decisions recorded; live Fast Gate/`/health` not claimable this turn
- exit criteria met: yes for Lead owns (priorities / fleet hierarchy / Fast Gate triage posture / coach / upgrades) within tool limits
- wrapper root cause (code): `~/.local/bin` wrappers symlink into repo `bin/`; `Path.resolve` follows into the workspace. Current tree trusts wrappers unless they sit under the role's **writable** roots (`_wrapper_source_is_agent_writable`); regression tests cover bin-outside-write ok vs write-root refuse. Lead write scope is `docs/planning` + `docs/ops` (+ `plans` dropped by contract intersect) — does not include `bin/`. Continuous Lane B fail implies stale control-plane process still using the old "any workspace wrapper" refuse, or a deploy that never loaded the writable-roots check. Residual Quinn risk: `watch-fast-gate.sh` lives under `scripts/ops/` while integrations write `scripts/` — if that name is on PATH, sandbox materialize will refuse for Quinn.
- fleet (authoritative roster + Lead check-in thread, not live API): Quinn last outcome completed; Rowan Gate 6 fail `run_801c2662885e`; Jules Codex/OpenAI key reject `run_1d177f7bd554` (ESCALATE); Reed Gate 6 fail (roster; check-in run id truncated); Mira prior continuous failed on approved-wrapper materialize
- acceptance evidence: `acceptance=pass · intent=lead_wrapper_retry · actor=lead-retry-receipt · summary=Root-caused approved-wrapper refuse vs writable-roots trust in tree; Reed owns CP reload/verify; Jules vault key escalate; Rowan+Reed Gate6 stay on their threads; live health/Fast Gate/unit probes blocked this turn; ship not approved`
- residual risks: continuous Lead dispatch may keep failing until control-plane runs the writable-roots-aware sandbox; Jules blocked without vault key; Rowan/Reed Gate 6 until their threads post passing acceptance; no live Fast Gate receipt this turn
- next gate unlocked: Reed verify/restart control-plane sandbox materialize; Rowan+Reed retry Gate 6 on their threads; Sir King restore Codex/OpenAI key for Jules
