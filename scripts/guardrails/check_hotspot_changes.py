#!/usr/bin/env python3
"""Fail when a critical hotspot changes without shrinking its ratchet budget."""
...
def main() -> int:
    ...
    for rel in critical_changed:
        path = ROOT / rel
        if not path.is_file():
            print(f"FAIL {rel}: critical hotspot deleted or missing")
            failed = True
            continue
        ...
        if error:
            ...
            print(
                f"WAIVED {rel}: active waiver until {waiver['expiry']} allows a "
                f"non-shrinking critical-hotspot change"
            )
