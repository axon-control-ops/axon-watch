...
echo
echo "==> Continuous voice reliability checklist (manual soak evidence)"
CHECKLIST_FILE="$ROOT/docs/VAXON_DESKTOP_VOICE_RELIABILITY_CHECKLIST.md"
mkdir -p "$(dirname "$CHECKLIST_FILE")"
{
  echo "# VAXON desktop voice reliability checklist"
  echo
  echo "Generated by \`scripts/desktop/prove-packaged-voice.sh\` at $(date -u +%Y-%m-%dT%H:%MZ)."
  echo
  echo "Automated portion (this run):"
  echo
  echo "- [x] Azure TTS available from live stack"
  echo "- [x] WebKitGTK 4.1 unlock gesture + Azure MP3 playback"
  echo
  echo "Operator soak (30 minutes on packaged Tauri — record PASS/FAIL):"
  echo
  echo "- [ ] Mic permission granted; privacy kill-switch stops capture immediately"
  echo "- [ ] Session-ready speech: unlock → one spoken greeting or status line"
  echo "- [ ] One hands-free turn: speak → reply → follow-up window reopens"
  echo "- [ ] Barge-in: say stop / wake while TTS plays; speech stops and listen resumes"
  echo "- [ ] Converse watchdog: deep turn either answers or times out with spoken fallback (no stuck thinking)"
  echo "- [ ] Runtime-unavailable fallback does not latch pending/thinking"
  echo "- [ ] No restart storm (ambient end backs off; failures use exponential backoff)"
  echo "- [ ] Sleep/reopen: after wake, hands-free re-arms without duplicate replies"
  echo
  echo "Diagnostics: use \`listVoiceLoopDiagnostics()\` / \`summarizeVoiceLoopDiagnostics()\` in the"
  echo "console (state, restart reason, latency, timeout/error counts — never audio/transcripts)."
  echo
  echo "Honesty: automated harness proves unlock+TTS engine only. Soak rows require a human operator."
} >"$CHECKLIST_FILE"
echo "wrote $CHECKLIST_FILE"

{
  echo
  echo "## Continuous voice reliability checklist ($(date -u +%Y-%m-%dT%H:%MZ))"
  echo
  echo "- Checklist written: \`docs/VAXON_DESKTOP_VOICE_RELIABILITY_CHECKLIST.md\`"
  echo "- Automated: Azure TTS + WebKitGTK unlock PASS"
  echo "- Manual soak rows remain open until operator evidence is recorded"
  echo
} >>"$EVIDENCE"
