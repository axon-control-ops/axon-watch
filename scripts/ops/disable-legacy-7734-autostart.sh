...
# Cloudflare named tunnel should target :4173 directly (no :7734 soft proxy).
set -euo pipefail
...
# Soft-cutover proxy is retired once Cloudflare ingress points at :4173.
systemctl --user disable --now axon-public-origin-proxy.service 2>/dev/null || true

# Stop a live :7734 listener if one is up (does not uninstall axon-local source).
...
echo "Legacy :7734 runtime disabled. Prefer: http://127.0.0.1:4173"
echo "Source retained for parity at repos/axon-local — do not autostart."
