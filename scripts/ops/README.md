...
| `axon-agent-terminal-job.sh` / `bin/axon-agent-terminal-job` | Enqueue Axon-owned PTY job (OTA/EAS live chat tee + vaxon); `--status` polls job |
| `watch-fast-gate.sh` | Poll Axon-X Fast Gate for the current branch / run id |
