...
const draft = ref('');
const modeOverride = ref<VaxonExecutiveComposerMode | null>(null);
const expanded = ref(false);
const composerInput = ref<HTMLTextAreaElement | null>(null);

const inferredMode = computed(() => inferVaxonComposerMode(draft.value));
const mode = computed(() => modeOverride.value ?? inferredMode.value);
...
function submit(content?: string): void {
  ...
  const message =
    content === undefined ? buildVaxonComposerSubmission(raw, mode.value) : raw;
  draft.value = '';
...
function toggleMode(): void {
  modeOverride.value = mode.value === 'mission' ? 'ask' : 'mission';
}
...
function handleKeydown(event: KeyboardEvent): void {
  if (
    shouldSubmitVaxonComposer(event) &&
    draft.value.trim() &&
    !props.pending
  ) {
...
<template>
  <div class="mc-live-ops__reply" :class="{ 'mc-live-ops__reply--expanded': expanded }">
    <form class="mc-live-ops__reply-form" @submit.prevent="submit()">
      <div class="mc-exec-composer__input-row">
        ...
        <textarea
          ...
          :disabled="pending"
          :aria-label="`${OPERATOR_PERSONA_NAME} command input`"
    ...
    <footer class="mc-exec-composer__context" aria-label="Command context">
      <button
        type="button"
        class="mc-exec-composer__mode"
        :disabled="pending"
        :title="`Switch from ${mode} mode`"
        @click="toggleMode"
      >
